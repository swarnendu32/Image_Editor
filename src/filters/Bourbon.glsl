precision highp float;
        ...
        void main() {
            ...
            vec3 mixed = mix(desaturated, tex.rgb, 1.0);
            vec4 color = vec4(mixed, tex.a);

            color.r += 0.0;
            color.b -= 0.0;

            color.rgb = (color.rgb - 0.5) * 1.259 + 0.5;
            ...
            color.rgb = color.rgb * pow(2.0, -0.195);

            float highlightAdjustment = pow(luminance, -0.038);
            ...
            if(canvasColorSpace < 0.5) {
                adjustedColor = vec3(color.r + -0.03125, color.g + -0.03125, color.b + -0.03125);
            }
            color.rgb = clamp(adjustedColor, 0.0, 1.0);
            ...
            if(true){
                color.rgb = vec3(color.r * 0.58 + color.g * 0.33 + color.b * 0.33);
            }
            
            float hazeColor = dot(color.rgb, vec3(1.0));
            float atmosphericLight = hazeColor * 0.038;
