...
void main() {
    ...
    vec3 mixed = mix(desaturated, tex.rgb, 1.31);
    vec4 color = vec4(mixed, tex.a);

    color.r += -0.044;
    color.b -= -0.044;

    color.rgb = (color.rgb - 0.5) * 1.2056666666666667 + 0.5;

    vec3 texColor = color.rgb;
    float luminance = dot(texColor, saturationVector);
    float adjustment = 1.0 + 0.05 * gaussian(luminance - 0.5);
    vec3 adjustedColor = texColor * adjustment * maxBrightness;
    color.rgb = clamp(adjustedColor, 0.0, 1.0);

    color.rgb = color.rgb * pow(2.0, 0.02);

    float highlightAdjustment = pow(luminance, -0.076);
    ...
    if(canvasColorSpace < 0.5) {
        adjustedColor = vec3(color.r + -0.05, color.g + -0.05, color.b + -0.05);
    }
    color.rgb = clamp(adjustedColor, 0.0, 1.0);
    ...
    float atmosphericLight = hazeColor * -0.026;
