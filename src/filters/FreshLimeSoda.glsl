...
void main() {
    ...
    color.rgb = (color.rgb - 0.5) * 0.8256666666666667 + 0.5;
    ...
    color.rgb = color.rgb * pow(2.0, 0.125);

    float highlightAdjustment = pow(luminance, -0.03);
    ...
    if(canvasColorSpace < 0.5) {
        adjustedColor = vec3(color.r + -0.0325, color.g + -0.0325, color.b + -0.0325);
    }
    color.rgb = clamp(adjustedColor, 0.0, 1.0);
    ...
    float atmosphericLight = hazeColor * -0.034;
