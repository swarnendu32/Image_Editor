...
void main() {
    ...
    color.rgb = (color.rgb - 0.5) * 1.2123333333333333 + 0.5;

    vec3 texColor = color.rgb;
    float luminance = dot(texColor, saturationVector);
    float adjustment = 1.0 + 0.07 * gaussian(luminance - 0.5);
    ...
    if(canvasColorSpace < 0.5) {
        adjustedColor = vec3(color.r + -0.0225, color.g + -0.0225, color.b + -0.0225);
    }
    color.rgb = clamp(adjustedColor, 0.0, 1.0);
