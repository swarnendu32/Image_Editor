...
void main() {
    ...
    if(canvasColorSpace < 0.5) {
        adjustedColor = vec3(color.r + 0.02625, color.g + 0.02625, color.b + 0.02625);
    }
    color.rgb = clamp(adjustedColor, 0.0, 1.0);
    ...
    float atmosphericLight = hazeColor * -0.006;
